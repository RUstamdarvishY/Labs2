...
Start:
	...
	case 2:
	{
		if (!list->begin || !list->begin->next) break;
		List *newStack = new List;
		ListElement *minElement = list->begin->next;
		...
		for (ListElement *tempPointer = list->begin->next; tempPointer != minElement; tempPointer = tempPointer->next)
			AddToBegin(newStack, tempPointer->value);
		list = newStack;
		break;
	}
	case 3:
