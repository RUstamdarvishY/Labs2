...
Start:
	...
	case 2:
	{
		if (!stack || !stack->next) break;
		Stack *newStack = NULL, *minElement = stack->next;
		for (Stack *tempPointer = stack->next; tempPointer; tempPointer = tempPointer->next)
			if (tempPointer->value < minElement->value) minElement = tempPointer;
		for (Stack *tempPointer = stack->next; tempPointer != minElement; tempPointer = tempPointer->next)
			Add(newStack, tempPointer->value);
		stack = newStack;
		break;
	}
	case 3:
